...
def test_organizer_reference_plans_replay_cleanly(public_cases):
    ...
    for case in public_cases:
        replay(case["input"], case["expected_output"]["directive_interpretation"], case["expected_output"], tol=0.01)
...
def test_seeded_random_feasible_cases_replay_cleanly():
    ...
    for i in range(2000):
        ...
        if i < 100:
            ...
            assert abs(oracle - result.total_cost_bdt) < 1e-4, (
                f"case {i}: LP {result.total_cost_bdt} vs oracle {oracle}"
            )
            oracle_checks += 1
    assert oracle_checks == 100
...
def test_infeasible_scenario_raises_not_crashes():
    ...
    req = ScenarioRequest.model_validate(
        dict(
            ...
            hours=[dict(hour=h, demand_kwh=50 if h == 0 else 0, solar_kwh=0, tariff_bdt_per_kwh=1) for h in range(24)],
        )
    )
    hours = req.hours_by_index()
